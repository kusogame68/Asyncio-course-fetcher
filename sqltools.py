...
class MyPsql:
    ...
    async def _target_db_exists(self) -> None:
        try:
            tdb_exists: bool = await self._conn.fetchval(
                    f"select * from pg_database where datname = '{self._target_db}';"
                ) is not None
    ...
    async def _target_user_exists(self) -> None:
        try:
            tuser_exists: bool = await self._conn.fetchval(
                    f"select * from pg_roles where rolname = '{self._target_user}';"
                ) is not None

            if not tuser_exists:
                console_log.info(f"{self._target_user} does not exist, create...")
                await self._conn.execute(
                        f"create role \"{self._target_user}\" with login password '{self._target_pwd}' \
    ...
    async def _target_schema_exists(self) -> None:
        try:
            tsch_exists: bool = await self._conn.fetchval(
                    f"select to_regnamespace('{self._target_sch}');"
                ) is not None

            if not tsch_exists:
                await self._conn.execute(
                        f"create schema if not exists \"{self._target_sch}\" authorization \"{self._target_user}\""
                    )
    ...
    async def _target_table_exists(self) -> None:
        try:
            ...
            if not ttable_exists:
                ...
                await self._conn.execute(
                        f"alter table \"{self._target_sch}\".\"{self._target_tb}\" owner to \"{self._target_user}\";"
                    )
    ...
    async def _grant_user(self) -> None:
        try:
            await self._conn.execute(
                    f"grant connect on database \"{self._target_db}\" to \"{self._target_user}\""
                )
