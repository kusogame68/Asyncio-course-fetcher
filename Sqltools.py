...
# so that when an asynchronous architecture becomes essential,
# the groundwork will already be in place.
#
...
class MyPsql:
    def __init__(self):
        ...
        # Credentials and targets (Loaded from environment)
    ...
    async def _target_db_exists(self) -> None:
        try:
            tdb_exists: bool = await self._conn.fetchval(
                    "select * from pg_database where datname = $1", self._target_db
                ) is not None

            if not tdb_exists:
                console_log.info(f"{self._target_db} does not exist, create...")
                await self._conn.execute(f"create database {_quote_ident(self._target_db)}")
        except Exception as e:
            console_log.error(f"Target db exists fail : {e}")

    async def _target_user_exists(self) -> None:
        try:
            tuser_exists: bool = await self._conn.fetchval(
                    "select * from pg_roles where rolname = $1;", self._target_user
                ) is not None

            if not tuser_exists:
                console_log.info(f"{self._target_user} does not exist, create...")
                await self._conn.execute(
                        f"create role {_quote_ident(self._target_user)} with login password $1 \
                            nosuperuser nocreatedb nocreaterole noinherit", self._target_pwd
                    )
        except Exception as e:
    ...
    async def _target_schema_exists(self) -> None:
        try:
            tsch_exists: bool = await self._conn.fetchval(
                    "select to_regnamespace($1);", self._target_sch
                ) is not None

            if not tsch_exists:
                await self._conn.execute(
                        f"create schema if not exists {_quote_ident(self._target_sch)} \
                            authorization {_quote_ident(self._target_user)}"
                    )
    ...
    async def _target_table_exists(self) -> None:
        try:
            ttable_exists: bool = await self._conn.fetchval(
                    "select to_regclass($1)", f'{self._target_sch}.{self._target_tb}'
                ) is not None

            if not ttable_exists:
                await self._conn.execute(f"""
                    create table if not exists {_quote_ident(self._target_sch)}.{_quote_ident(self._target_tb)} (
                ...
                await self._conn.execute(
                        f"alter table {_quote_ident(self._target_sch)}.{_quote_ident(self._target_tb)} \
                            owner to {_quote_ident(self._target_user)};"
                    )
    ...
    async def _grant_user(self) -> None:
        try:
            await self._conn.execute(
                    f"grant connect on database {_quote_ident(self._target_db)} to {_quote_ident(self._target_user)}"
                )
    ...
    async def _set_trigger(self) -> None:
        ...
        try:
            q_sch = _quote_ident(self._target_sch)
            q_tb = _quote_ident(self._target_tb)
            await self._conn.execute(f"""
                create or replace function {q_sch}.set_updated_at()
                returns trigger as $$
                begin
                    new.updated_at := now();
                    return new;
                ...
                execute function {q_sch}.set_updated_at();
            """)
        ...
        try:
            ...
            sql: str = f"""
                ...
                do update set
                    ...
                    updated_at = now()
                where (
                    {q_sch}.{q_tb}."Mon" is distinct from excluded."Mon" or
                    {q_sch}.{q_tb}."Tue" is distinct from excluded."Tue" or
                    {q_sch}.{q_tb}."Wed" is distinct from excluded."Wed" or
                    {q_sch}.{q_tb}."Thr" is distinct from excluded."Thr" or
                    {q_sch}.{q_tb}."Fri" is distinct from excluded."Fri"
                )
            """
    ...
    async def fetch_sql(self) -> pd.DataFrame:
        try:
            sql: str = f"""
                            ...
                            from {_quote_ident(self._target_sch)}.{_quote_ident(self._target_tb)}
                        """
